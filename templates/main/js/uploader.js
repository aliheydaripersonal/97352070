...
(function(factory) {
  ...
  var defaults = {
    ...
    onRemove: function(){},   //params: id,media_id
  ...
  DmUploader.prototype.initDnD = function()
  {
    ...
    $(document).on("click","#"+widget.settings.listId+" .remove_upload_file", function(evt) {
		widget.remove($(evt.target).closest('.remove_upload_file').attr("target-id"));
    });
  };
  ...
  DmUploader.prototype.remove = function(id)
  {
		this.settings.uploaded_files_count--;
		if (this.settings.uploaded_files_count < this.settings.max) {
			$("#" + this.settings.boxId).show();
		}
    var media_id = $("#"+id+" .remove_upload_file").attr("media-id");
		this.settings.onRemove.call(this.element, id,media_id);
  };
...
function ui_multi_add_file(list,id, file) {
  ...
  $(list).find('li.empty').fadeOut(); // remove the 'no files yet'
  $(list).prepend(template);
  $('#uploaderFile' + id+" .remove_upload_file").attr("target-id",'uploaderFile' + id);
}
function ui_multi_add_already_file(list,id, url) {
    var template = $('#files-template').text();
    template = template.replace('%%filename%%', url);
    ...
    $(list).find('li.empty').fadeOut(); // remove the 'no files yet'
    $(list).prepend(template);
    $('#uploaderFile' + id+" .remove_upload_file").attr("target-id",'uploaderFile' + id).attr("media-id",id);
}
function ui_multi_remove_upload_file(list,id) {
    $('#' + id).remove();
}
